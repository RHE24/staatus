...
if(isset($_GET['column']) && isset($_GET['order']))
{
	...
	{
		if($order['column'] == "players" || $order['column'] == "version" || $order['column'] == "ratings" || $order['column'] == "records")
		{
			switch($order['column'])
			{
				case 'players':
				...
				case 'version':
					if($order['order'] == "desc") {
						$order['set'] = "SELECT * FROM ".$DB['prefix']."servers ORDER BY status DESC, premium DESC, server_version DESC";
					}else{
						$order['set'] = "SELECT * FROM ".$DB['prefix']."servers ORDER BY status DESC, premium DESC, server_version ASC";
					}
				break;
...
?>
<?php
if($_GET['column'] == "version" && $_GET['order'] == "desc")
{
	echo '<th>Versioon&nbsp;<a href="?page=servers&column=version&order=asc"><img src="style/img/bullet_arrow_down.png" /></a></th>';
}
elseif($_GET['column'] == "version" && $_GET['order'] == "asc")
{
	echo '<th>Versioon&nbsp;<a href="?page=servers&column=version&order=desc"><img src="style/img/bullet_arrow_up.png" /></a></th>';
}
else
{
	echo '<th><a href="?page=servers&column=version&order=desc">Versioon</a></th>';
}
?>
<?php
...
$query = mysql_query(''.$order['set'].'') or die(mysql_error());
			...
			if ($number_of_rows == 0){                
			echo '<tr>';                
			echo '<td colspan="5" align="center">Pole saadaval</td>';                
			echo '</tr>';            
			} else {                
