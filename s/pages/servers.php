...
if(isset($_GET['game_id']))
{
	...
	{
		//echo $_GET['game_id'];
		$order['filter'] = " WHERE game_id = ".mysql_real_escape_string($_GET['game_id'])."";
	}
}

$order['set'] = "SELECT * FROM ".$DB['prefix']."servers".$order['filter']." ORDER BY status DESC, premium DESC, server_players DESC, server_maxplayers DESC";
	...
	if($order['order'] == "asc" || $order['order'] == "desc")
	{
		if($order['column'] == "players" || $order['column'] == "ratings" || $order['column'] == "records")
		{
			switch($order['column'])
			{
				case 'players':
					if($order['order'] == "desc") {
						$order['set'] = "SELECT * FROM ".$DB['prefix']."servers".$order['filter']." ORDER BY status DESC, premium DESC, server_players DESC";
					}else{
						$order['set'] = "SELECT * FROM ".$DB['prefix']."servers".$order['filter']." ORDER BY status DESC, premium DESC, server_players ASC";
					}
				break;
				case 'ratings': // võtta tabelist ratings hinnanguid ja liita kokku ning järjestada hinnangu järgi
					if($order['order'] == "desc") {
						$order['set'] = "SELECT *, (SELECT SUM(rating) FROM ".$DB['prefix']."ratings WHERE ".$DB['prefix']."ratings.server_id = ".$DB['prefix']."servers.id) AS totalRating FROM ".$DB['prefix']."servers".$order['filter']." ORDER BY status DESC, premium DESC, totalRating DESC";
					}else{
						$order['set'] = "SELECT *, (SELECT SUM(rating) FROM ".$DB['prefix']."ratings WHERE ".$DB['prefix']."ratings.server_id = ".$DB['prefix']."servers.id) AS totalRating FROM ".$DB['prefix']."servers".$order['filter']." ORDER BY status DESC, premium DESC, totalRating ASC";
					}
				break;
				case 'records':
					if($order['order'] == "desc") {
						$order['set'] = "SELECT * FROM ".$DB['prefix']."servers".$order['filter']." ORDER BY status DESC, premium DESC, server_maxplayers DESC";
					}else{
						$order['set'] = "SELECT * FROM ".$DB['prefix']."servers".$order['filter']." ORDER BY status DESC, premium DESC, server_maxplayers ASC";
					}
				break;
			}
		}
        ...
        <div class="span12">
          ...
          <div class="row-fluid">
            <div class="span12">
              ...
              <p>
<?php		  
$query = mysql_query("SELECT * FROM ".$DB['prefix']."games") or die(mysql_error());
$number_of_rows = mysql_num_rows($query);            
if ($number_of_rows != 0){   
	echo '<form action="" method="get"><table class="table table-bordered"><tr><input type="hidden" name="page" value="servers">';
	while($row = mysql_fetch_array($query))                
	{   
		echo '<td align="center">';
		echo '<input type="radio" name="game_id" value="'.$row['id'].'"> <img src="'.$row['img_path'].'" alt="'.$row['game_name'].'" /> '.$row['game_name'].'';
		echo '</td>';
	}
	echo '<td align="center"><input type="submit" value="Filtreeri"></td>';
	echo '</tr></table></form>';
}
?>
...
{
	$gfe = '&game_id='.$_GET['game_id'].'';
}
if($_GET['column'] == "players" && $_GET['order'] == "desc")
{
	echo '<th>Mängijad&nbsp;<a href="?page=servers&column=players&order=asc'.$gfe.'"><img src="style/img/bullet_arrow_down.png" /></a></th>';
}
elseif($_GET['column'] == "players" && $_GET['order'] == "asc")
{
	echo '<th>Mängijad&nbsp;<a href="?page=servers&column=players&order=desc'.$gfe.'"><img src="style/img/bullet_arrow_up.png" /></a></th>';
}
else
{
	echo '<th><a href="?page=servers&column=players&order=desc'.$gfe.'">Mängijad</a></th>';
}
?>
<?php
if($_GET['column'] == "records" && $_GET['order'] == "desc")
{
	echo '<th>Rekordarv&nbsp;<a href="?page=servers&column=records&order=asc'.$gfe.'"><img src="style/img/bullet_arrow_down.png" /></a></th>';
}
elseif($_GET['column'] == "records" && $_GET['order'] == "asc")
{
	echo '<th>Rekordarv&nbsp;<a href="?page=servers&column=records&order=desc'.$gfe.'"><img src="style/img/bullet_arrow_up.png" /></a></th>';
}
else
{
	echo '<th><a href="?page=servers&column=records&order=desc'.$gfe.'">Rekordarv</a></th>';
}
?>
<?php
if($_GET['column'] == "ratings" && $_GET['order'] == "desc")
{
	echo '<th>Hinnang&nbsp;<a href="?page=servers&column=ratings&order=asc'.$gfe.'"><img src="style/img/bullet_arrow_down.png" /></a></th>';
}
elseif($_GET['column'] == "ratings" && $_GET['order'] == "asc")
{
	echo '<th>Hinnang&nbsp;<a href="?page=servers&column=ratings&order=desc'.$gfe.'"><img src="style/img/bullet_arrow_up.png" /></a></th>';
}
else
{
	echo '<th><a href="?page=servers&column=ratings&order=desc'.$gfe.'">Hinnang</a></th>';
}
?>
<th>Kaart</th>
</tr>
